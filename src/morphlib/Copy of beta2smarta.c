...
#include "Copy of beta2smarta.proto.h"
...
beta2smarta(char *source, unsigned char *res)
{
	beta2mac(source,res,SMARTA);
}

beta2smk(char *source, unsigned char *res)
{
	beta2mac(source,res,SMK);
}

beta2mac(char *source, unsigned char *res, int xlit)
{
	 char * sp;
	unsigned char * rp;
	int acc;
