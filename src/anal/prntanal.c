...
DumpPerseusAnalysis(gk_word *Gkword, PrntFlags prntflags, gk_analysis *anal, FILE *fout, int cura)
{
	char tmp[LONGSTRING];
...
DumpEndingIndex(gk_word *Gkword, PrntFlags prntflags, gk_analysis *anal, FILE *fout, int cura)
{
	...
	if( strcmp(workword_of(anal),rawword_of(anal))) fprintf(fout," %s", rawword_of(anal) );
